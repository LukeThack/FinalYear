...
data_augmentation = keras.Sequential([layers.RandomFlip("horizontal"),layers.RandomZoom(0.3),layers.RandomTranslation(0.2, 0.2),layers.RandomContrast(0.3),])

model=models.Sequential([data_augmentation,resnet_model,layers.GlobalAveragePooling2D(),layers.Dense(64, activation="relu"),layers.Dropout(0.5),layers.Dense(1, activation="sigmoid")])
model.compile(optimizer=keras.optimizers.Adam(learning_rate=1e-4),loss="binary_crossentropy",metrics=["accuracy"])#optimiser set for transfer learning, binary_crossentropy, best loss function for binary outputs.
...
first_model=model.fit(image_train,label_train,validation_data=(image_test,label_test),epochs=20,callbacks=callbacks,batch_size=16)
...
ship_model=model.fit(image_train,label_train,validation_data=(image_test, label_test),epochs=20,batch_size=16,callbacks=callbacks)
